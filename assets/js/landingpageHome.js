...
function testScroll(ev){
    if(window.pageYOffset>250)
    {
        ...
        $('.signupicon').css({"display":"none"});

    }
    if(window.pageYOffset<250)
    {
        $('.learn-more').css({"display":"block"});
        $('.signupicon').css({"display":"block"});
    }
...
function validate() {
    ...
    if($('#Formname').val()==="" || !$('#Formname').val().match(/^[a-zA-Z]+$/) )
    {
    ...
    if($('#emailId').val()==="" || !$('#emailId').val().match(/^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]{2,4}$/))
    {
	 ...
	 if($('#organisationId').val()==="" || !$('#organisationId').val().match(/[a-zA-Z]{1,50}$/) )
    {
        ...
        $('#errororg').css({"display":"block"});
        return false;
    }
    else{
        $('#errororg').css({"display":"none"});
